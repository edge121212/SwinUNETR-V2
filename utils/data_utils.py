...
def get_loader(args):
    ...
    all_data = json_data.get("training", [])
    ...
    block_size = num_samples // 10
    blocks = [all_data[i * block_size : (i + 1) * block_size] for i in range(9)]
    blocks.append(all_data[9 * block_size :])
    
    test_blocks = [fold * 2, fold * 2 + 1]
    val_block = [(fold * 2 + 2) % 10]
    ...
    is_ct = args.task.lower() in ["lung", "pancreas", "task06_lung", "task07_pancreas"]
    ...
    if args.test_mode:
        ...
        if args.use_normal_dataset:
            train_ds = data.Dataset(data=train_data, transform=train_transform)
        else:
            train_ds = data.CacheDataset(
                data=train_data, transform=train_transform, cache_num=24, cache_rate=1.0, num_workers=args.workers
            )
        ...
        val_ds = data.Dataset(data=val_data, transform=val_transform)
