...
class AttentionGate(nn.Module):
    """Additive attention gate (Attention U-Net, Oktay et al. 2018), 2D/3D.

    Gates a skip-connection feature ``x`` using a gating signal ``g`` that has the
    same spatial size and channel count (here, the upsampled decoder feature). It
    learns a per-voxel attention map in [0, 1] and reweights the skip feature,
    suppressing background and emphasising small foreground structures (e.g. the
    ...
    def __init__(self, spatial_dims: int, channels: int) -> None:
        ...
        self.w_x = conv_type(channels, inter_channels, kernel_size=1, bias=True)
        self.w_g = conv_type(channels, inter_channels, kernel_size=1, bias=True)
        self.psi = conv_type(inter_channels, 1, kernel_size=1, bias=True)
        self.relu = nn.ReLU(inplace=True)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x: torch.Tensor, g: torch.Tensor) -> torch.Tensor:
        attn = self.sigmoid(self.psi(self.relu(self.w_x(x) + self.w_g(g))))
        return x * attn


class AttnUnetrUpBlock(UnetrUpBlock):
    """``UnetrUpBlock`` variant that gates its skip connection with an :class:`AttentionGate`.

    Behaves exactly like ``UnetrUpBlock`` except the skip feature is filtered by an
    attention gate (gated by the upsampled decoder feature) before the channel-wise
    concatenation. The gate operates on ``out_channels`` because both the upsampled
    decoder feature and the skip feature have ``out_channels`` at the concat point.
    """
    ...
    def __init__(
        ...
        self.attn_gate = AttentionGate(spatial_dims, out_channels)

    def forward(self, inp, skip):
        out = self.transp_conv(inp)
        skip = self.attn_gate(skip, out)
...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        # decoderN maps to attention-gate "level" N (1 = highest resolution / nearest output,
        # 5 = deepest / coarsest). A level listed in attn_gate_levels uses AttnUnetrUpBlock;
        # otherwise the stock UnetrUpBlock. Empty -> all stock -> state_dict identical to MONAI.
        gate_levels_set = {int(level) for level in (attn_gate_levels or ())}

        def _make_up_block(level: int, **kwargs):
            block_cls = AttnUnetrUpBlock if level in gate_levels_set else UnetrUpBlock
            return block_cls(**kwargs)

        self.decoder5 = _make_up_block(
            5,
            spatial_dims=spatial_dims,
            in_channels=16 * feature_size,
